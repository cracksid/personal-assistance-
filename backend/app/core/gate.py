...
class ToolGate:
    ...
    def __init__(self, memory: MemoryStore | None = None) -> None:
        ...
        # Handed to tools through ToolContext. Optional so tests can build a
        # gate without loading an embedding model.
        self._memory = memory
    ...
    async def invoke(
        ...
        if tool is None:
            # Record the miss. When the model reaches for a capability that
            # does not exist, that is a feature request generated by real
            # use -- far better evidence of what to build next than guessing.
            # Query it with:
            #   SELECT tool_name, COUNT(*) FROM audit_log
            #   WHERE status='unknown_tool' GROUP BY tool_name;
            await self._audit_unknown_tool(db, tool_name, raw_args, user_id)
            known = ", ".join(t.name for t in registry.all_tools())
            logger.info("Model asked for a tool that does not exist: %r", tool_name)
            return ToolResult(
    ...
    async def _execute(
        ...
        # Built here, not by the tool. A tool never opens its own database
        # session or decides which user it is acting for.
        context = ToolContext(db=db, user_id=user_id, memory=self._memory)
    ...
    async def _audit_unknown_tool(
        self, db: Session, tool_name: str, raw_args: dict, user_id: int | None
    ) -> None:
        """
        Log a request for a capability JARVIS does not have.

        Nothing ran, so this is not an execution record -- it is a record of
        something the user wanted and could not get.
        """
