...
A personal AI assistant that runs on your own machine. It talks and listens,
remembers what you tell it, reads the web, works with your files, acts on a
schedule, watches folders, takes plugins, and lives in a desktop window with a
tray icon.

Single user, local, Windows. Not a hosted service.

```
368 tests · 86% coverage · 8,750 lines of application code
```
...
| | |
|---|---|
| **Chat** | Streaming replies, in a desktop app or a browser tab |
| **Voice** | Hold to talk (Whisper, locally); optional spoken replies (Piper, locally) |
| **Memory** | Facts extracted from conversation, searched by meaning, inspectable and deletable |
| **Files** | Read, write, search, delete — inside a sandbox, destructive actions gated |
| **Web** | Search and read pages, with an SSRF guard and untrusted-content labelling |
| **Vision** | Screenshots, image questions, and local OCR |
| **Automation** | Reminders, recurring tasks that run themselves, folder watchers |
| **Apps** | Open installed programs by name — never by command line |
| **Plugins** | Drop a `.py` file in a folder; it becomes a tool |

## Getting started

```bash
pip install -r backend/requirements-dev.txt
copy .env.example .env          # then add your API key, or set LLM_PROVIDER=ollama
cd backend && python -m alembic upgrade head
cd ../frontend && npm install && npm run build
cd ../desktop && npm install && npm start
```

That last command opens the desktop app, which starts the backend itself. For
development without Electron, see [Run the server](#run-the-server) and
[The interface](#the-interface-phase-13a).

## Documentation

| | |
|---|---|
| **[docs/architecture.md](docs/architecture.md)** | How it is put together and why — start here |
| **[docs/plugins.md](docs/plugins.md)** | Writing your own tools |
| **[docs/security-review.md](docs/security-review.md)** | What was attacked, what broke, what held |
| **`CLAUDE.md`** | Locked decisions, the phase plan, and the full build log |

## The two rules everything follows

**One choke point for anything dangerous.** Every tool execution passes through
a single gate in `core/gate.py`. A tool declares whether it needs approval and
describes what it is about to do; the core decides. No tool implements its own
"are you sure?", so a forgotten check in tool number forty cannot delete a
folder.

**Arrows point downward.** Interfaces → API → core → capabilities, providers,
memory. A lower tier never imports from a higher one. `memory/` does not know
an LLM exists; `providers/` does not know what an assistant is. Only `core/`
does.
...
- [Setup (Phase 2)](#setup-phase-2)
- [Choosing a model (Phase 5)](#choosing-a-model-phase-5)
- [Run the server](#run-the-server)
- [Run the tests](#run-the-tests)
- [Database (Phase 4)](#database-phase-4)
- [Internet tools (Phase 10)](#internet-tools-phase-10)
- [Reminders and scheduling (Phase 11a)](#reminders-and-scheduling-phase-11a)
- [Scheduled tasks (Phase 11b)](#scheduled-tasks-phase-11b)
- [The interface (Phase 13a)](#the-interface-phase-13a)
- [Opening applications](#opening-applications)
- [Voice in the UI (Phase 7 → finally reachable)](#voice-in-the-ui-phase-7--finally-reachable)
- [Security review (Phase 19)](#security-review-phase-19)
- [The desktop app (Phase 14)](#the-desktop-app-phase-14)
- [Settings and memory (Phase 13b)](#settings-and-memory-phase-13b)
- [Plugins (Phase 12)](#plugins-phase-12)
- [File watchers (Phase 11c)](#file-watchers-phase-11c)
- [File system tools (Phase 9a)](#file-system-tools-phase-9a)
- [Vision (Phase 8)](#vision-phase-8)
- [Voice (Phase 7a)](#voice-phase-7a)
- [Memory (Phase 6)](#memory-phase-6)
...
python -m pytest --cov        # with coverage, currently 86%
```

Settings live in `backend/pyproject.toml` rather than on the command line,
including a `fail_under = 80` ratchet — a guard against regression, not a
target to chase.

**Worth knowing what that number cannot tell you.** Every serious bug found
in this project so far lived in code the suite already executed: tests
writing to the real database, chat resuming a scheduled task's thread, a
failed tool rendering as a blank line, a fact stored with a broken escape
sequence, a socket that never reconnected. Coverage shows what is *never
run*. It says nothing about whether what runs is right. All five were found
by using the thing.

What it is genuinely good for is pointing at unexercised logic. It put the
provider adapters lowest — the message reshaping between JARVIS's neutral
format and each vendor's wire format, which needs no network to test and
had almost none. Writing those tests turned up a latent bug where the last
frame of a streamed reply could be dropped.

The speech, OCR and vision adapters sit at 32–45% deliberately. Testing them
means faking faster-whisper, Piper and onnxruntime, and the tests would
mostly assert that the mocks behave as written. Coverage without
verification is worse than a visible gap, because it looks like safety.
